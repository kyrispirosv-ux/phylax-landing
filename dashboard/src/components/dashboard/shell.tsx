  ...
  return (
    <div className="min-h-screen bg-[#0B0F1A]">
      ...
      <header className="sticky top-0 z-50 bg-[#0B0F1A]/90 backdrop-blur-xl border-b border-white/[0.05]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Link
              ...
              className="relative w-11 h-11 rounded-full bg-white/[0.05] flex items-center justify-center hover:bg-white/[0.08] active:scale-95 transition"
              aria-label="Alerts"
            >
            ...
            <Link
              ...
              className="w-11 h-11 rounded-full bg-white/[0.05] flex items-center justify-center hover:bg-white/[0.08] active:scale-95 transition"
              aria-label="Settings"
            >
              <svg className="w-[18px] h-[18px] text-white/50" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            ...
            <button
              ...
              className="md:hidden w-11 h-11 rounded-full bg-white/[0.05] flex items-center justify-center active:scale-95 transition-transform"
            >
              <svg className="w-[18px] h-[18px] text-white/50" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                {menuOpen ? (
        ...
        {/* Mobile nav dropdown — touch-friendly 44px+ targets */}
        {menuOpen && (
          <nav className="md:hidden border-t border-white/[0.05] px-4 py-2 space-y-0.5">
            {NAV_ITEMS.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                ...
                className={`flex items-center min-h-[44px] px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                  pathname === item.href ? "bg-white/[0.08] text-white" : "text-white/40 active:bg-white/[0.04]"
                }`}
              >
                {item.label}
              </Link>
