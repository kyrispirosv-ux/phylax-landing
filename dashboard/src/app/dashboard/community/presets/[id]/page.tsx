...
export default function PresetDetailPage() {
  ...
  if (loading) {
    return <div className="flex justify-center py-12"><div className="w-6 h-6 border-2 border-white/25 border-t-emerald-400 rounded-full animate-spin" /></div>;
  }
  if (!preset) return <p className="text-center text-white/40 py-12">Preset not found.</p>;

  return (
    <div className="max-w-3xl mx-auto">
      <Link href="/dashboard/community/presets" className="text-white/40 hover:text-white/50 text-sm mb-4 inline-block transition-colors">
        ...
        <div className="flex items-start justify-between mb-4">
          ...
          {preset.rating_count > 0 && (
            <div className="flex items-center gap-1 text-amber-400">
              ...
              <span className="text-white/40 text-xs">({preset.rating_count})</span>
            </div>
          )}
        </div>

        {preset.description && <p className="text-white/50 text-sm mb-4">{preset.description}</p>}
        ...
        <div className="space-y-2 mb-6">
          ...
          {(preset.rules || []).map((r: any, i: number) => (
            <div key={i} className="flex items-center gap-2 bg-white/[0.03] rounded-lg px-3 py-2">
              ...
              <span className="text-white/60 text-sm">{r.text}</span>
              {r.target && <span className="text-white/40 text-xs ml-auto">{r.target}</span>}
            </div>
          ))}
        </div>
      ...
      <div className="mt-6">
        ...
        <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-4 mb-4">
          <div className="flex items-center gap-1 mb-3">
            {[1, 2, 3, 4, 5].map((s) => (
              <button key={s} onClick={() => setReviewRating(s)} className="p-0.5">
                <svg className={`w-5 h-5 ${s <= reviewRating ? "text-amber-400" : "text-white/25"}`} fill="currentColor" viewBox="0 0 24 24">
          ...
          <textarea
            ...
            className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-emerald-500/50 resize-none h-16 mb-2"
          />
          <button
          ...
          {reviews.map((r) => (
            <div key={r.id} className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-4">
              <div className="flex items-center gap-2 mb-1">
                <div className="flex">
                  {[1, 2, 3, 4, 5].map((s) => (
                    <svg key={s} className={`w-3.5 h-3.5 ${s <= r.rating ? "text-amber-400" : "text-white/25"}`} fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
                    </svg>
                  ))}
                </div>
              ...
              {r.body && <p className="text-white/50 text-sm">{r.body}</p>}
            </div>
          ))}
        </div>
      </div>
