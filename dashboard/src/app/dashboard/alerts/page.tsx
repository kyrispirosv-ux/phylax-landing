    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        {unreadCount > 0 && (
          <button
            onClick={markAllRead}
            className="text-xs text-white/40 hover:text-white/50 transition"
          >
            Mark all read
          </button>
        )}
      ...
      <div className="space-y-2">
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 mt-2">
                {alert.domain && (
                  <span className="text-xs text-white/25">{alert.domain}</span>
                )}
                {alert.reason_code && (
                  <span className="text-xs text-white/25 bg-white/[0.03] px-2 py-0.5 rounded">
                    {alert.reason_code}
                  </span>
                )}
              </div>
        ...
        {alerts.length === 0 && (
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-8 text-center">
            <p className="text-white/40 text-sm">No alerts yet. Phylax is monitoring.</p>
          </div>
        )}
      </div>
    </div>
